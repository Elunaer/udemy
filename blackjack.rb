...
def generate_deck(card_numbers)
  card_numbers.each do |cn|
    @deck_template.each_key do |s|
      @deck_template[s] << "#{cn} #{s}"
    end
  end
...
def hit_or_stay
  ...
  when 'hit'
    ...
    #{@player_cards.flatten} and dealer has the following cards: #{@dealer_cards.flatten}"
    round_scores
  when 'stay'
    @player_stay = true
    dealer_turn
...
def dealer_turn
  ...
  if @dealer_cards_value < 17
    ...
    puts "Dealer has the following cards: #{@dealer_cards.flatten} and
    player has the following cards: #{@player_cards.flatten}"
    dealer_turn
...
def continue_playing
  abort("You've ran out of money this time, I think we'll call it quits, come back another time!") if @currency.zero?
...
puts "Thank you for joining us in Blackjack, normal rules apply.
Get as close to 21 without going bust and beat the dealer (computer).
We've been kind and given you a balance of £100 to start you off Let's do this!!"
currency
round
